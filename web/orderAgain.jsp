<%-- 
    Document   : orderAgain
    Created on : July 2, 2024, 8:46:04 AM
    Author     : ASUS
--%>
                        ...
                        <div class="rounded shadow p-3 bg-body">
                            ...
                            <table class="table text-center default-cursor">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                    <td>
                                        <c:url var="linkItems" value="ItemDetailController">
                                            <c:param name="pid" value="${C.value.item.id}"></c:param>
                                        </c:url>
                                        <a href="${linkItems}" class="text-decoration-none">${C.value.item.id}</a>
                                    </td><input type="hidden" name="productId" value="${C.value.item.id}">
                                    <td>
                                        <a href="${linkItems}">
                                            <img src="${C.value.item.imgPath}" width="100">
                                        </a>
                                    </td>
                                    <td>${C.value.item.name}</td>
                                    <td>$${C.value.item.price}</td>
                                    <td>${C.value.quantity}</td>
                                    <td>$${C.value.item.price * C.value.quantity}</td>
                                    <c:set var="total" scope="request" value="${total + C.value.item.price * C.value.quantity}"/>
                                    </tr>
                                </c:forEach>
                                </tbody>
                            </table>
